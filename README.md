...
# SOCRadar Sentinel Webhook Connector
...
[![Deploy to Azure](https://aka.ms/deploytoazurebutton)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FRadargoger%2Fsocradar-sentinel-alarm-connector-api%2Fmain%2FAlarm%20_Receiver.json)
...
- `sentinelResourceGroupName` - Resource group containing Sentinel
- `sentinelSubscriptionId` - Subscription ID

## Post-Deployment

1. Go to API Connections > `azuresentinel-webhook-connection`
2. Click Edit API connection > Authorize > Save
3. Get webhook URL from Logic App > Overview > Workflow URL
4. Add the URL to SOCRadar webhooks

## Field Mapping

| SOCRadar | Sentinel |
|----------|----------|
| alarm_id | Title (SOCRadar-AlarmID-{id}) |
...
curl -X POST "<WEBHOOK_URL>" \
  ...
  -d '{"data":[{"alarm_id":12345,"alarm_risk_level":"HIGH","alarm_text":"Test"}]}'
```
